...
> **NOTE:**
> Direct3D 12 is currently an Experimental feature. To enable, you must set `OLLAMA_D3D12=1` for the Ollama server as
described in the [FAQ](faq#how-do-i-configure-ollama-server)

Additional GPU support on Windows is provided via Direct3D 12. It uses Shader Model 6.6, which requires Windows 10 21H1 or newer and a D3D12 Feature Level 12_0 GPU. Most GPUs released since around 2017 meet these requirements.

Ollama enumerates Direct3D 12 adapters through DXGI and reports VRAM with `IDXGIAdapter3::QueryVideoMemoryInfo` so the scheduler can make optimal decisions.

> **NOTE:**
> The Direct3D 12 backend is at an early stage. Most operations are not yet implemented and will fall back to CPU. It is intended for development and benchmarking, not production use. It can be useful for early testing on Windows machines without CUDA or ROCm.

### GPU Selection

To select specific Direct3D 12 GPU(s), you can set the environment variable
`GGML_D3D12_VISIBLE_DEVICES` to a comma-separated list of numeric adapter IDs on
the Ollama server as described in the [FAQ](faq#how-do-i-configure-ollama-server).
If you encounter any problems with Direct3D 12 based GPUs, you can disable all
Direct3D 12 GPUs by setting `GGML_D3D12_VISIBLE_DEVICES=-1`
