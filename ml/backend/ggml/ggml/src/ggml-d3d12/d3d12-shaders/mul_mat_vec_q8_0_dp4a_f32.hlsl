...
uint pack_q8_0(uint block_off, uint group) {
    const uint off = block_off + QS_OFFSET + group * 4u;
    return load_u8(src0_buf, off) |
           (load_u8(src0_buf, off + 1u) << 8u) |
           (load_u8(src0_buf, off + 2u) << 16u) |
           (load_u8(src0_buf, off + 3u) << 24u);
}
...
void main(uint3 gid : SV_GroupID, uint3 gtid : SV_GroupThreadID) {
    ...
    float acc = 0.0f;
    for (uint block = gtid.x; block < num_blocks; block += TG) {
