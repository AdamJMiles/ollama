#define TG_SIZE 256
#define QK 256u
#define BLOCK_SIZE 210u
#define QL_OFFSET 0u
#define QH_OFFSET 128u
#define SCALES_OFFSET 192u
#define D_OFFSET 208u
...
int load_i8(RWByteAddressBuffer buf, uint off) {
    const uint v = load_u8(buf, off);
    return int(v) - ((v >= 128u) ? 256 : 0);
}
...
void main(uint3 dtid : SV_DispatchThreadID) {
    ...
    const uint group = elem >> 7u;
    const uint rel = elem & 127u;
    const uint quadrant = rel >> 5u;
    const uint l = rel & 31u;
    const uint ql_index = group * 64u + ((quadrant & 1u) * 32u) + l;
    const uint packed = load_u8(src_buf, block_off + QL_OFFSET + ql_index);
    const uint low4 = (quadrant < 2u) ? (packed & 0x0Fu) : (packed >> 4u);
    const uint hbits = (load_u8(src_buf, block_off + QH_OFFSET + group * 32u + l) >> (quadrant * 2u)) & 3u;
    const int q = int(low4 | (hbits << 4u)) - 32;
    const int sc = load_i8(src_buf, block_off + SCALES_OFFSET + group * 8u + quadrant * 2u + (l >> 4u));
    const float d = load_f16(src_buf, block_off + D_OFFSET);
    const float v = d * float(sc * q);
