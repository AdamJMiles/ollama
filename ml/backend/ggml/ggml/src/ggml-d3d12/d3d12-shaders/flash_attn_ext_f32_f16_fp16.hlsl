...
half k_load(uint d, uint j, uint h_kv) {
    return load_f16h(k_buf, k_off + j * k_nb1 + h_kv * k_nb2 + d * 2u);
}

half v_load(uint d, uint j, uint h_kv) {
    return load_f16h(v_buf, v_off + j * v_nb1 + h_kv * v_nb2 + d * 2u);
}
