...
uint load_u32_unaligned(RWByteAddressBuffer buf, uint off) {
    return load_u8(buf, off) | (load_u8(buf, off + 1u) << 8u) | (load_u8(buf, off + 2u) << 16u) | (load_u8(buf, off + 3u) << 24u);
}
...
void main(uint3 dtid : SV_DispatchThreadID) {
    ...
    const uint packed = load_u8(src_buf, block_off + QS_OFFSET + (elem & 15u));
    const uint low4 = (elem < 16u) ? (packed & 0x0Fu) : (packed >> 4u);
    const uint qh = load_u32_unaligned(src_buf, block_off + QH_OFFSET);
    const uint high = ((qh >> elem) & 1u) << 4u;
    const float d = load_f16(src_buf, block_off);
    const float m = load_f16(src_buf, block_off + 2u);
    const float v = float(low4 | high) * d + m;
