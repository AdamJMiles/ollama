...
void main(uint3 gid : SV_GroupID, uint3 gtid : SV_GroupThreadID) {
    ...
    for (uint k0 = 0; k0 < K; k0 += TK) {
        for (uint i = tid; i < TM * TK; i += TM * TN) {
            ...
            const uint gk = k0 + lk;
            sa[lm][lk] = (gm < M && gk < K) ? load_f16h(src0_buf, src0_base + gm * src0_nb1 + gk * 2u) : (half)0.0f;
        }
        ...
        [unroll]
        for (uint k = 0; k < TK; k += 4u) {
            const half4 a = half4(sa[gtid.y][k], sa[gtid.y][k + 1u], sa[gtid.y][k + 2u], sa[gtid.y][k + 3u]);
            const float4 b = float4(sb[k][gtid.x], sb[k + 1u][gtid.x], sb[k + 2u][gtid.x], sb[k + 3u][gtid.x]);
            acc += (float)a.x * b.x + (float)a.y * b.y + (float)a.z * b.z + (float)a.w * b.w;
        }
        GroupMemoryBarrierWithGroupSync();
    }
