...
void main(uint3 gtid : SV_GroupThreadID, uint3 gid : SV_GroupID) {
    ...
    for (uint j = tid; j < n_pairs; j += TG_SIZE) {
        ...
        const uint i0 = j;
        const uint i1 = j + n_pairs;
        const float x0 = asfloat(src_buf.Load(src_row + i0 * 4u));
        const float x1 = asfloat(src_buf.Load(src_row + i1 * 4u));

        dst_buf.Store(dst_row + i0 * 4u, asuint(x0 * cos_th - x1 * sin_th));
        dst_buf.Store(dst_row + i1 * 4u, asuint(x0 * sin_th + x1 * cos_th));
    }
