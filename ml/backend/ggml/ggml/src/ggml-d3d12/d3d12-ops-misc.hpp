...
inline bool dispatch_misc(dispatch_ctx & ctx, const ggml_tensor * node) {
    ...
    if (node->op == GGML_OP_ARANGE) {
        ...
        uint32_t off[1] = { 0 };
        if (!ctx_bind_raw_uavs_sliding(ctx, uavs, 1, &uav_table, off)) return true;
        if ((off[0] % 4) != 0) return true;
    ...
    UINT consts[5] = { count, off[0], off[1], p0, p1 };
